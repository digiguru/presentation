...
const decks = manifest.map(presentation => ({
  file: presentation.url,
  expectsGpt: Boolean(presentation.capabilities?.gptInput),
}));
...
try {
  await new Promise((resolve, reject) => {
    server.on('request', (req, res) => {
      serve(req, res, server.missingRequests).catch(error => {
        console.error(error);
  ...
  const { stdout: indexDom } = await execFileAsync(chrome, [
    ...
    '--disable-sync', '--metrics-recording-only', '--virtual-time-budget=1000', '--dump-dom',
  ...
  for (const deck of decks) {
    ...
    try {
      const { stdout } = await execFileAsync(chrome, [
        ...
        `http://127.0.0.1:${address.port}/${deck.file}`,
