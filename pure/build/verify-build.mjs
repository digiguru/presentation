...
const forbiddenRuntimePatterns = [
  ...
  /js\/gpt-component\.(?:js|html)/i,
  /js\/pie-component\.(?:js|css)/i,
  /window\.Reveal\b/,
  /https:\/\/ai-prompt-writer\.vercel\.app\/api\//i,
];
...
async function textFiles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...await textFiles(filePath));
    else if (/\.(?:html|css|js|json)$/i.test(entry.name)) files.push(filePath);
  }
  return files;
}
...
for (const [file, content] of Object.entries(expectedDecks)) {
  ...
  assert.match(html, content, `${file} should contain its source deck content`);
...
for (const filePath of await textFiles(dist)) {
  const source = await readFile(filePath, 'utf8');
  for (const forbidden of forbiddenRuntimePatterns) {
    assert.doesNotMatch(source, forbidden, `${path.relative(dist, filePath)} contains old-runtime reference ${forbidden}`);
  }
}
...
for (const forbiddenPath of [
  ...
  ['plugin'],
  ['js', 'gpt-component.js'],
  ['js', 'gpt-component.html'],
  ['js', 'pie-component.js'],
  ['js', 'pie-component.css'],
]) {
  await assert.rejects(access(path.join(dist, ...forbiddenPath)), `Pure must not ship ${forbiddenPath.join('/')}`);
}

console.log(`Verified Pure-only Reveal 6 build ${buildInfo.commitSha} and ${referencedAssets.size} referenced presentation assets.`);
